...
// shared UI helpers
const StatusNode = ({p}) => {
  const td=String(p?.today||"").trim(), th=String(p?.thru||"").trim();
  if (looksTime(td)||looksTime(th)) {
    return (
  ...
  const lab=statusFor(p);
  return lab ? (
    <span className={`px-2 py-0.5 rounded-full text-[10px] font-semibold whitespace-nowrap ${lab.toUpperCase().includes('FIN')? 'bg-emerald-600/15 text-emerald-300 border border-emerald-700/40':'bg-indigo-500/15 text-indigo-300 border border-indigo-700/40'}`}>{lab}</span>
  ) : (
...
// ===== data =====
const ENDPOINTS = [
  "/api/espn?kind=leaderboard",
  "/api/espn?kind=scoreboard",
];
const TEAMS = {
...
export default function App(){
  ...
  const [muted,setMuted]=useState(()=>{try{return JSON.parse(localStorage.getItem("muted")||"true");}catch{return true;}}), audioCtxRef=useRef(null), lastSoundRef=useRef(0);

  // blip helpers
  const blipColor=(n,f)=>blip[nk(n)+":"+f]||null;
  ...
  const rowsFull=useMemo(()=>teams.map((t,i)=> (
    <tr key={t.team} className={(i%2?"bg-slate-900/40":"bg-slate-900/20")+( (blipColor(t.p1.name,'toPar')==='red'||blipColor(t.p1.name,'today')==='red'||blipColor(t.p2.name,'toPar')==='red'||blipColor(t.p2.name,'today')==='red')?" rowPulseRed": (blipColor(t.p1.name,'toPar')==='green'||blipColor(t.p1.name,'today')==='green'||blipColor(t.p2.name,'toPar')==='green'||blipColor(t.p2.name,'today')==='green')?" rowPulseGreen":"") }>
      ...
      <td className={`px-3 py-2 text-left ${blipColor(t.p1.name,'toPar')?'flipScore':''}`} style={blipStyle(t.p1.name,'toPar')}><span className="score-badge"><ScoreBadge value={totalFromRounds(t.p1)} /></span></td>
      ...
      <td className="px-3 py-2 whitespace-nowrap"><div className="wStatus text-left"><StatusNode p={t.p1}/></div></td>
      {/* G2 */}
      <td className="px-3 py-2 whitespace-nowrap"><span className="hover:text-sky-300 nameClip">{t.p2.name}</span></td>
      <td className={`px-3 py-2 text-left ${blipColor(t.p2.name,'toPar')?'flipScore':''}`} style={blipStyle(t.p2.name,'toPar')}><span className="score-badge"><ScoreBadge value={totalFromRounds(t.p2)} /></span></td>
    ...
    <div className="min-h-screen w-full bg-slate-950 text-slate-100 p-6">
      <div className={`mx-auto max-w-6xl mainWrap ${announceStack.length? 'blurActive':''}`}>
        <style>{`
          ...
          .tbl-compact .metrics{display:grid;grid-template-columns:58px 58px 90px;column-gap:10px;align-items:center;justify-items:start}
          /* compact column titles */
          .tbl-compact .compactHead{display:flex;justify-content:space-between;align-items:center}
          .tbl-compact .compactHead .labels{display:grid;grid-template-columns:58px 58px 90px;column-gap:10px;color:#94a3b8;font-size:11px;justify-items:start}
          .tbl-compact .wScore,.tbl-compact .wScoreLabel{width:auto;text-align:left;display:inline-flex;justify-content:flex-start}
          .tbl-compact .wToday,.tbl-compact .wTodayLabel{width:auto;text-align:right;display:inline-flex;justify-content:flex-end}
          .tbl-compact .wStatus,.tbl-compact .wStatusLabel{width:auto;text-align:left;display:inline-flex;justify-content:flex-start;white-space:nowrap}
        `}</style>
